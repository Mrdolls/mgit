...
static void print_usage(void) {
    printf("%s%smgit - Lightweight Git Workflow Automation CLI (v%s)%s\n\n", ANSI_BOLD, ANSI_CYAN, MGIT_VERSION, ANSI_RESET);
    printf("%sUSAGE:%s\n", ANSI_BOLD, ANSI_RESET);
    printf("  mg <command> [options]\n\n");
    printf("%sAVAILABLE COMMANDS:%s\n", ANSI_BOLD, ANSI_RESET);
    printf("  %spush [message]%s   Automate git add ., git commit -m [message] and git push\n", ANSI_GREEN, ANSI_RESET);
    printf("                     (Default commit message: \"Auto push\")\n");
    ...
    printf("%sEXAMPLES:%s\n", ANSI_BOLD, ANSI_RESET);
    printf("  mg push \"Fix user authentication bug\"\n");
    printf("  mg show\n");
    printf("  mg update\n\n");
}
