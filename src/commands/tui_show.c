...
static void print_modal_field(int left_pad, const char *label, const char *value, const char *val_color, int card_w) {
    for (int i = 0; i < left_pad; i++) printf(" ");

    printf("%s│%s", ANSI_BRIGHT_BLUE, ANSI_RESET);

    int inner_w = card_w - 2;
    int val_w = inner_w - 10;
    if (val_w < 5) val_w = 5;

    printf(" %s%s%-6.6s%s %s:%s %s%s%-*.*s%s",
           ANSI_BOLD, ANSI_BRIGHT_WHITE, label, ANSI_RESET,
           ANSI_BRIGHT_WHITE, ANSI_RESET,
           ANSI_BOLD, val_color, val_w, val_w, value, ANSI_RESET);
...
static void draw_tui(CommitInfo *commits, int count, int selected, const char *branch,
    ...
    if (action_menu_open) {
        ...
        /* Row 3a: DESC in Yellow */
        print_modal_field(left_pad, "DESC", commits[selected].subject, ANSI_BRIGHT_YELLOW, card_w);

        /* Row 3b: ID in Green */
        print_modal_field(left_pad, "ID", commits[selected].hash, ANSI_BRIGHT_GREEN, card_w);

        /* Row 3c: Author in Blue */
        print_modal_field(left_pad, "Author", commits[selected].author, ANSI_BRIGHT_CYAN, card_w);

        /* Row 3d: Date in Blue */
        print_modal_field(left_pad, "Date", commits[selected].date, ANSI_BRIGHT_CYAN, card_w);
